...
public class CustomSimpleAdapter extends SimpleAdapter {
	...
	private void bindView(int position, View view) {
        ...
        LinearLayout domainLayout = BaseViewHolder.get(view, R.id.domain_layout);
        domainLayout.removeAllViews();
        Object allDomain = dataSet.get("allDomain");
        if(allDomain != null){
        	String[] domains = dataSet.get("allDomain").toString().split(" ");
            int maxLength = 5;
            LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(DensityUtil.dip2px(mContext, 18), DensityUtil.dip2px(mContext, 18));
            for (int i=0;i<domains.length && i<maxLength;i++) {
    			ImageView imageView = new ImageView(mContext);
    			layoutParams.setMargins(0,0,DensityUtil.dip2px(mContext, 4),0);
    			imageView.setLayoutParams(layoutParams);
    	        int drawS = mContext.getResources().getIdentifier(domains[i],"domain", mContext.getPackageName());
